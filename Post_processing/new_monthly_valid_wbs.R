...
setwd('/home/karunpandit/wbs_long_e') # Define working directory
...
test<- testdata[,8]/1000*365
month3<- as.character(testdata[,3])
test_av <-aggregate(test~month3,FUN=mean)
test_ava<-test_av[1,]
test_avb<-test_av[2:4,]
test_avc<-test_av[5:12,]
test_av_n<-rbind(test_avb,test_ava,test_avc)
...
test_av1_n<-rbind(test_av1b,test_av1a,test_av1c)
...
mall<-cbind(abc_2[,1],test_av_n[,2],test_av1_n[,2],abc_2[,2])
...
legend(0,2, legend=c("Observation at WBS in 2015","Observation for WBS in 2017","Simulation with best parameters for WBS"), fill =c("#1b9e77","#7570b3", "#d95f02"),cex=0.65, xjust=0,x.intersp=0.1, pt.cex=1,y.intersp=1.1, bty="n")
