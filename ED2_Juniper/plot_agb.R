...
s1gd<-read.csv('C:/Karun_documents/juniper/outputs_point_juniper/out_def/annual_agb_co1901_2020.csv')
s1gd[,8]<- s1gd[,3]+s1gd[,4] + s1gd[,6]

s1ga<-read.csv('C:/Karun_documents/juniper/outputs_point_juniper/out_allom/annual_agb_co1901_2020.csv')
s1ga[,8]<- s1ga[,3]+s1ga[,4] + s1ga[,6]

s1gap<-read.csv('C:/Karun_documents/juniper/outputs_point_juniper/out_allompar/annual_agb_co1901_2020.csv')
s1gap[,7]<- s1gap[,3]+s1gap[,4] + s1gap[,5]+ s1gap[,6]
s1gap[,8]<- s1gap[,3]+s1gap[,4] + s1gap[,6]

s2gd<-read.csv('C:/Karun_documents/juniper/outputs_point_juniper/twopft6def/annual_agb_co_1901_2020.csv')
s2gd[,8]<- s2gd[,3]+s2gd[,4] + s2gd[,6]

s2ga<-read.csv('C:/Karun_documents/juniper/outputs_point_juniper/twopft6allom/annual_agb_co_1901_2020.csv')
s2ga[,8]<- s2ga[,3]+s2ga[,4] + s2ga[,6]

s2gap<-read.csv('C:/Karun_documents/juniper/outputs_point_juniper/twopft6allompar/annual_agb_co_1901_2020.csv')
s2gap[,8]<- s2gap[,3]+s2gap[,4] + s2gap[,6]



graphics.off()
plotname1 <- paste0("AGB_all_annual.png")
...
plot3<- plot(x=s2gap[,2],y = s1gap[,4], type="l",xaxt="n",yaxt="n",xlab="year",
             ylab=vvv,cex.lab=1.6,ylim=ylts,col=juncol)
#par(new=TRUE)
xvals<-seq(0,120,20)
yvals<-seq(0,1.5,0.25)
...
graphics.off()
plotname1 <- paste0("AGB_all_legend.png")
